...
class Root(object):
    ...
    @cherrypy.expose
    def index(self, **args):
        self.templateVars = copy.deepcopy(
            self.defaultTemplateVars)  # create a deepcopy so other people's changes aren't cached
    ...
    def plot(self, **args):
        ...
        p = self.getPlot(args)
        d = model.Plot()
        buffer = d.getPlotPath(p, imgformat=imgformat)
        cherrypy.response.headers['Content-Type'] = "image/{}".format("svg+xml" if imgformat == "svg" else "png")
    ...
    def table(self, **args):
        ...
        for col in df.columns:
            html = html.replace('<th>{}'.format(col),
